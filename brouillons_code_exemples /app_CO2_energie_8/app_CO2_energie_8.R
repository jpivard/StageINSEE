...
server <- function(input, output) {
    ...
    #Figure sur la répartition de la consommation d'énergie par sources en France
    
    output$conso_sources_energies_plot<- renderPlotly({
        dfshiny12<- df_7_long_bis
        conso_sources_energies_plot <- ggplot(dfshiny12, aes(x=Annee,y=value, color=source))+ 
        geom_bar(aes (x=Annee, y =value, fill=source),stat = "identity", position = "stack")+
        scale_fill_manual(values = c("black", "orange","brown","blue","green"),labels = c("Pétrole","Nucléaire","Charbon","Gaz","Hydroélectricité"))+
        labs(x="Année",y="Mtoe")+
        theme_gray()
