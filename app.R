...
load(file="data/conso_enr_prop.rds")
load(file="data/conso_enr_valeur.rds")
load(file="data/prod_enr_prop.rds")
load(file="data/prod_enr_valeur.rds")
load(file = "data/donnees_Kaya.rds")
load(file="data/Emissions_CO2_globales.rds")
load(file="data/Emissions_CO2_secteurs.rds")
load(file="data/intensite_carbone_energie.rds")
load(file="data/intensite_carbone_PIB.rds")
load(file="data/Investissements_par_secteur_I4CE.rds")
load(file="data/Investissements_Plan_I4CE.rds")
load(file="data/part_ER_conso.rds")
load(file="data/part_ER_prod.rds")
load(file="data/conso_energie_sources_Fr.rds")
load(file="data/prod_nucleaire_Fr_All.rds")
load(file="data/prod_sources_energie_France.rds")
load(file="data/prod_totale_energie.rds")
load(file="data/deseq_conso_prod.rds")
...
ui <-  dashboardPage(
    ...
    dashboardSidebar(
        width = 350,
        sidebarMenu(
            menuItem("Emissions de CO2", icon = icon("industry"),
                     menuSubItem("Emissions globales", tabName = "CO2_global"),
                     menuSubItem("Emissions par secteurs", tabName = "CO2_secteurs")
                     ),
            menuItem("L'énergie en France", icon = icon("atom"),
                     menuSubItem("Energies de tous types", tabName = "energie_1"),
                     menuSubItem("Energies décarbonées", tabName = "energie_2")
                      ),
            menuItem("Investissements climat", icon = icon("dashboard"),
                     menuSubItem("Par secteurs", tabName = "inv_1"),
                     menuSubItem("Par investisseurs", tabName = "inv_2")
                     ),
            menuItem("Finance verte", tabName = "fin_verte", icon = icon("comment-dollar"),
                     menuSubItem("Financements des investissements par financeurs", tabName = "fin_verte_1"),
                     menuSubItem("Aperçu du marché des fonds verts", tabName = "fin_verte_2")
                     ),
    ...
    dashboardBody(
        ...
        tabItems(
            ...
            tabItem(
                ...
                tabBox(
                    title = "Figures 8 : Investissements annuels par secteurs",em("Source :I4CE"),p("Lecture : Ces graphiques récapitulent pour trois périodes successives le montant par secteurs des différents investissements publics pour le climat. L'un des secteurs dans lequel l'Etat investit le plus est celui des infrastructures ferroviaires. "),
                    width = 8,
